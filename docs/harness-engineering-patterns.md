...
IBM 研究跑 4,705 次 agent 后发现：
- 小模型（7-20B）在 T=0 时 100% 确定性——但准确率只有 33-42%
- 大模型（120B+）在 T=0 时只有 **12.5%** 确定性
- **确定性和准确性的相关性 r = -0.11**——两者无关

所以对你的项目：
- 用 `temperature=0 + fixed seed + JSON schema` 做第一层
- 用 `Pass^k` 指标（不是 `Pass@k`）——要求 k 次运行**全部**产出相同决策，而不是"最优的那次对了就行"
- 对合规关键决策（如最终仓位建议），用小模型做一致性验证
